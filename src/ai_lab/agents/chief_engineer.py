"""Chief Engineer / Orchestrator agent — plans tasks, does not solve everything alone."""
...
class ChiefEngineerAgent(BaseAgent):
    ...
    system_prompt = (
        ...
        "and schedule specialists. You do NOT solve the whole problem yourself. "
        "Return JSON only."
    )
    ...
    async def run(self, task: TaskSpec, ctx: AgentContext) -> AgentResult:
        ...
        # Persist understanding artifact
        ...
        # Synthesis stage: write final_report.md when asked
        if "synthesis" in task.objective.lower() or task.state_context and task.state_context.value == "SYNTHESIS":
            report = (
                "# Final report\n\n"
                f"## Summary\n{payload.get('summary', '')}\n\n"
                f"## Understanding\n{payload.get('understanding', '')}\n\n"
                f"## Open unknowns\n"
                + "\n".join(f"- {u}" for u in (payload.get("unknowns") or []))
                + "\n\n"
                "_Generated by Chief Engineer synthesis. Verification/red-team artifacts "
                "remain authoritative for claim status._\n"
            )
            await ctx.tools.call(
                "files.write",
                allowed=ctx.allowed_tools_for(self.role, task),
